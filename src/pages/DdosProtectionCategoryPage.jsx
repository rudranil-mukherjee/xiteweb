...
import { ShieldCheck, Zap, Layers, Globe, ArrowRight, HelpCircle, Info } from 'lucide-react';
import AnimatedParticles from '@/components/AnimatedParticles';

const protectionCategoriesData = {
  'game-server': {
...
const DdosProtectionCategoryPage = () => {
  const { category } = useParams();
  const categoryData = protectionCategoriesData[category];

  if (!categoryData) {
    return <div className="text-center py-20 text-white">DDoS Protection category not found. <Link to="/ddos-protection" className="text-pink-400 hover:underline">Go back</Link></div>;
  }
  ...
  return (
    <div className="min-h-screen pb-16 relative overflow-hidden">
      <Helmet>
        <title>{`${categoryData?.name || 'DDoS Protection'} | XiteNodes India`}</title>
        <meta name="description" content={categoryData?.description || 'Enterprise DDoS protection solutions from XiteNodes India. Protect your servers with up to 192Tbps mitigation capacity.'} />
        <link rel="canonical" href={`https://xitenodes.com/ddos-protection/${category}`} />
        <meta property="og:title" content={`${categoryData?.name || 'DDoS Protection'} | XiteNodes India`} />
        <meta property="og:description" content={categoryData?.description || 'Enterprise DDoS protection solutions from XiteNodes India.'} />
        <meta property="og:url" content={`https://xitenodes.com/ddos-protection/${category}`} />
        <meta property="og:type" content="website" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${categoryData?.name || 'DDoS Protection'} | XiteNodes India`} />
      </Helmet>
      ...
      <div className="container mx-auto px-6 py-12 md:py-20 relative z-10">
        ...
        >
          ...
          <h1 className={`text-5xl md:text-6xl font-extrabold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-${categoryData.color}-500 via-${categoryData.color}-400 to-${categoryData.color}-300`}>
            {categoryData.name}
          </h1>
        ...
        <motion.div 
          ...
          {categoryData.plans.map((plan) => (
            <PlanCard key={plan.name} plan={plan} categoryColor={categoryData.color} />
          ))}
        </motion.div>
        ...
        >
          ...
          <p className="text-gray-300 mb-6">
           ...
           <Button asChild size="lg" className={`bg-gradient-to-r from-${categoryData.color}-600 to-${categoryData.color}-500 hover:from-${categoryData.color}-700 hover:to-${categoryData.color}-600 text-white text-lg`}>
        ...
        >
           <Button asChild size="lg" variant="outline" className={`border-gray-500 text-gray-400 hover:bg-gray-500 hover:text-white`}>
             <Link to="/ddos-protection">View All DDoS Protection <Layers className="ml-2 w-5 h-5" /></Link>
           </Button>
        </motion.div>
      </div>
    </div>
...
export default DdosProtectionCategoryPage;
